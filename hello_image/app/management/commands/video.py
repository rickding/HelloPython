...
ffmpeg = r'D:\work\ffmpeg\bin\ffmpeg.exe'
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.main('video/video.mp4', save=False, iscut=False, start='00:00:00', end='00:00:14')
    ...
    # 将图片合成视频
    def jpg2video(self, outfile_name, fps):
    ...
    # 调用ffmpeg获取mp3音频文件
    def video2mp3(self, file_name, outfile_name):
        cmdstr = " -i {0} -f mp3 {1} -y".format(file_name, outfile_name)
        self.cmd(cmdstr)

    # 合成音频和视频文件
    def video_add_mp3(self, file_name, mp3_file, outfile_name):
        cmdstr = " -i {0} -i {1} -strict -2 -f mp4 {2} -y".format(file_name, mp3_file, outfile_name)
        self.cmd(cmdstr)

    # 视频截取
    def video_cut(self, file_name, outfile_name, start, end):
    ...
    # 主函数
    def main(self, video, save=False, iscut=False, start='00:00:00', end='00:00:14'):
        """
        :param video: 原视频文件地址
        ...
        """
        file_cut = video.split('.')[0] + '_cut.mp4'
        file_mp3 = video.split('.')[0] + '.mp3'
        file_temp_avi = video.split('.')[0] + '_temp.avi'
        outfile_name = video.split('.')[0] + '-code.mp4'

        print("开始生成...")
        if iscut:
            print("正在截取视频...")
            self.video_cut(video, file_cut, start, end)
            video = file_cut

        print("正在转换代码图片...")
        vc = self.video2txt_jpg(video)  # 视频转图片，图片转代码图片
        fps = vc.get(cv2.CAP_PROP_FPS)  # 获取帧率
        vc.release()

        print("正在分离音频...")
        self.video2mp3(video, file_mp3)  # 从原视频分离出 音频mp3
